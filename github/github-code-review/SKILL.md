...
- Authenticated with GitHub (`gh auth login`; token setup: GitHub settings -> tokens)
...
```bash
# env: export GITHUB_TOKEN=... GH_OWNER=... GH_REPO=...
...
In addition to inline comments, leave a top-level summary so the PR author gets the full picture at a glance. Use the review output format from review-output-template.md.
...
## Reference: review-output-template.md
